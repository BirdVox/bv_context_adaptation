...
opts{1}.time.gamma_bounds = [1 (1+8*Q)];
% Disable chunking
...
% Gammatone wavelets (asymmetric) instead of Morlet (symmetric).
opts{2}.banks.time.handle = @gammatone_1d;
% No constraint on the maximum scale.
opts{2}.banks.time.max_scale = Inf;
% Only retain modulation frequencies below 64 Hz.
opts{2}.banks.time.gamma_bounds = [10 Inf];
